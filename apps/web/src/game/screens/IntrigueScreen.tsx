...
export function IntrigueScreen({ view, me }: { view: GameView; me: Character }) {
  ...
  const spymaster = me.council?.spymaster.characterId ? view.characters[me.council.spymaster.characterId] : undefined;
    ...
    <ScreenFrame title="Intrigue" icon="☾" wide>
      <div className="intrigue-grid">
        <section>
          ...
          {mine.length === 0 && <p className="muted">Aucun complot en cours. Clic droit sur un personnage ou sa fiche → Comploter.</p>}
          {mine.map((s) => {
            ...
            return (
              <div key={s.id} className="scheme-card">
                <div className="row" style={{ gap: 10 }}>
                  ...
                  <ActionButton className="btn-sm btn-ghost" onClick={() => act({ type: 'scheme.cancel', payload: { schemeId: s.id } }, 'Complot abandonné')}>
                    Abandonner
                  </ActionButton>
                </div>
                ...
                <div className="scheme-stats muted num">
                  ...
                  <span className={schemeDiscoveryChance(s) > 0.1 ? 'neg' : ''}>Découverte {Math.round(schemeDiscoveryChance(s) * 100)} %/mois</span>
                </div>
                {s.agents.length > 0 && (
                  <div className="row" style={{ gap: 4, marginTop: 4 }}>
                    ...
                    {s.agents.map((a) => (
                      <Portrait key={a} c={view.characters[a]} view={view} size={24} onClick={() => openCharacter(a)} />
                    ))}
                  </div>
                )}
                {s.discoveredBy.length > 0 && <div className="neg" style={{ fontSize: 12 }}>Découvert par {s.discoveredBy.map((d) => view.characters[d]?.firstName).join(', ')}</div>}
              </div>
            );
          })}
          {approaches.length > 0 && (
          ...
          {against.map((s) => (
            <div key={s.id} className="scheme-card hostile">
              <div className="row" style={{ gap: 10 }}>
                <Portrait c={view.characters[s.ownerId]} view={view} size={40} onClick={() => openCharacter(s.ownerId)} />
                <div className="grow">
                  <div className="display">{t(`scheme.${s.type}`)}</div>
                  <div className="soft">ourdi par {charName(view, view.characters[s.ownerId])}</div>
                </div>
                <button className="btn btn-sm btn-danger" onClick={() => useUi.setState({ contextMenu: { x: window.innerWidth / 2, y: window.innerHeight / 2, characterId: s.ownerId } })}>
                  Réagir
                </button>
              </div>
        ...
        <section>
          ...
          {known.length === 0 && <p className="muted">Lancez « Découvrir des secrets » ou confiez cette tâche à votre maître-espion.</p>}
          {known.map((s) => (
            <div key={s.id} className="info-line">
              <span>
                <button className="link-btn" onClick={() => openCharacter(s.ownerId)}>
              ...
              {s.exposed ? (
                ...
                <ActionButton className="btn-sm btn-ghost" onClick={() => act({ type: 'secret.expose', payload: { secretId: s.id } }, 'Le scandale éclate')}>
                  Révéler
                </ActionButton>
              )}
            </div>
          ...
          {hooks.length === 0 && <p className="muted">Un levier force la main : mariage, alliance ou vassalité acceptés malgré les réticences.</p>}
          {hooks.map((h) => (
          ...
          {own.map((s) => (
            <div key={s.id} className="info-line">
              ...
              <span className={s.exposed ? 'neg' : s.knownBy.length > 1 ? 'neg' : 'muted'}>{s.exposed ? 'révélé' : `connu de ${Math.max(0, s.knownBy.length - 1)} personne(s)`}</span>
            </div>
          ))}
        </section>
      </div>
