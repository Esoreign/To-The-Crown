...
import { PROVINCE_GEO, armyMen, armyUpkeep, availableLevies, capitalProvinceOf, militaryStrength, sideOf, warsOf } from '@ttc/game-core';
  ...
  return (
    <ScreenFrame title="Armées" icon="⚔" wide>
      <div className="military-summary">
        ...
        <ActionButton className="btn-primary" disabled={!cap} onClick={() => act({ type: 'army.raise', payload: {} }, 'Les levées se rassemblent', 'war')}>
          ⚔ Lever l’ost
        </ActionButton>
      </div>
      <div className="intrigue-grid">
        <section>
          ...
          {armies.length === 0 && <p className="muted">Aucune armée levée. Les levées en campagne coûtent de l’or chaque mois.</p>}
          {armies.map((a) => (
            <button
              key={a.id}
              className="title-row"
            ...
            >
              <span className="grow">
                {a.commanderId ? view.characters[a.commanderId]?.firstName : 'Sans commandant'} · {PROVINCE_GEO[a.location]?.name}
              </span>
          ...
          {wars.map((w) => {
            ...
            return (
              <button key={w.id} className="title-row" onClick={() => openWar(w.id)}>
                <span className="grow">
                  {t(`cb.${w.cb}`)} : {view.characters[w.attackerId]?.firstName} contre {view.characters[w.defenderId]?.firstName}
                </span>
        ...
        <section>
          ...
          <p className="muted" style={{ fontSize: 12.5, marginTop: 0 }}>
            Troupes professionnelles permanentes, plus efficaces que les levées. Elles rejoignent chaque armée levée.
          </p>
          <div className="setting-row">
            <label htmlFor="maa-men">Effectif</label>
            <input id="maa-men" type="range" min={100} max={1000} step={100} value={men} onChange={(e) => setMen(Number(e.target.value))} />
                ...
                <Tip
                  content={() => (
                    <div>
                      ...
                      <div>Dégâts {u.damage} · Robustesse {u.toughness} · Poursuite {u.pursuit} · Écran {u.screen}</div>
                      {u.siege ? <div>Siège {u.siege}</div> : null}
                      {u.counters?.length ? <div className="pos">Contre : {u.counters.map((c) => t(`unit.${c}`)).join(', ')}</div> : null}
                      {u.goodTerrain?.length ? <div className="pos">À l’aise : {u.goodTerrain.map((x) => tOr(`terrain.${x}`, x)).join(', ')}</div> : null}
                      {u.badTerrain?.length ? <div className="neg">Gêné : {u.badTerrain.map((x) => tOr(`terrain.${x}`, x)).join(', ')}</div> : null}
                      <div className="muted">Entretien : {u.upkeep} or / 100 hommes / mois</div>
                    </div>
                  )}
                >
                  <span className="grow" tabIndex={0}>
                    <span aria-hidden="true">{contentGlyph(u.icon)}</span> {t(`unit.${u.id}`)} <span className="muted num">({fmt(me.maa[u.id] ?? 0)})</span>
                  </span>
                </Tip>
                <ActionButton className="btn-sm" disabled={me.gold < cost} onClick={() => act({ type: 'army.recruit', payload: { unit: u.id as Exclude<UnitType, 'levy'>, men } }, `${men} ${t(`unit.${u.id}`)} recrutés`, 'coin')}>
