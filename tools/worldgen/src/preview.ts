...
const [w0, s0, e0, n0] = (bboxArg ?? '-180,-60,180,84').split(',').map(Number) as [
  number,
  number,
  number,
  number,
...
const prov = JSON.parse(
  fs.readFileSync('/home/user/To-The-Crown/apps/web/public/world/provinces.geojson', 'utf8'),
);
const borders = JSON.parse(
  fs.readFileSync('/home/user/To-The-Crown/apps/web/public/world/borders.geojson', 'utf8'),
);
...
for (const f of prov.features) {
  ...
  for (const poly of polys)
    for (const ring of poly) {
      d += 'M' + ring.map((p: number[]) => `${X(p[0]!).toFixed(1)},${Y(p[1]!).toFixed(1)}`).join('L') + 'Z';
    }
  svg += `<path d="${d}" fill="hsl(${h},35%,${55 + (f.id % 3) * 7}%)" fill-rule="evenodd"/>`;
}
for (const f of borders.features)
  svg += `<path d="M${f.geometry.coordinates.map((p: number[]) => `${X(p[0]!).toFixed(1)},${Y(p[1]!).toFixed(1)}`).join('L')}" stroke="#222" stroke-width="0.6" fill="none"/>`;
svg += '</svg>';
