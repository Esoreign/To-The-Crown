...
import { pause, register, resolveEvents, startSolo } from './helpers';

test('guerre : lever l’ost, suivre la guerre en cours, déplacer une armée', async ({ page }) => {
  await register(page, 'guerre');
  await startSolo(page, 'ott'); // Bayezid Ier : le siège de Constantinople a déjà commencé
  ...
  const box = await page.locator('.game-map .maplibregl-canvas').boundingBox();
...
test('déclaration de guerre : la fenêtre présente casus belli ou raison', async ({ page }) => {
  ...
  await startSolo(page, 'cas'); // Enrique III de Castille
  ...
  await page.getByTestId('search-input').fill('Martin');
  await page.locator('.title-row', { hasText: 'Martin' }).first().click();
  await expect(page.getByTestId('character-panel')).toContainText('Martin');
